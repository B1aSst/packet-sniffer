...
trames = rdpcap("network-samples/www-total.pcapng")
    ...
    if trame.haslayer("TCP") and trame.haslayer("Raw"):
        if (trame[0][1].dport) == 80 or (trame[0][1].sport) == 80:
            ...
            if "Authorization: Basic" in data:
                ...
                print("\n")
                print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print(f"^*^ Un utilisateur est connecté")
                print(f"^*^ Client : {trame[0][1].dst} Serveur : {trame[0][1].src}")
                print(f"^*^ Host : {host[-1]}")
                print(f"^*^ Username : {credentials[-0][-0]}")
                print(f"^*^ Password : {credentials[-0][-1]}")
                print("\n")


for trame in trames:
    check_login_http(trame)
